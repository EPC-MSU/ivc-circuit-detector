...
* `parameters_variations.json` - Описание перебора параметров компонентов
  ...
  * Раздел `filter` содержит обязательные настройки фильтрации граничных условий:
    * `bounds_extension_percentage` - на сколько процентов расширяются границы диапазона параметров при генерации граничных схем. Например, при значении 30.0 нижняя граница смещается на 30% ниже минимального значения, верхняя — на 30% выше максимального.
